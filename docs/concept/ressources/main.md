...
The `Ressource<RessourceType>` concept in Feline Engine is designed to manage various types of resources used in the engine, such as textures, fonts, shaders, ... This concept provides a unified way to handle different resource types while ensuring type safety and efficient resource management.

## Overview
A `Ressource` is a template class that takes a `RessourceType` as a template parameter. Every 'Ressource' are manage by the `RessourceManager` class, which is responsible for loading, unloading, and caching resources to optimize performance and memory usage. This manager ensures that resources are only loaded once and shared across different parts of the engine as needed.

> [!NOTE]
> See [ressource_manager](./ressource_manager.md) for further infomations about the management.
...
- `Texture`: Represents image resources used for rendering sprites and backgrounds. It can be in various formats : PNG, JPEG, BPM, SVG, ... See [here for more detail](./ressourceType/texture.md)
- `Atlas`: Represents texture atlas resources used for optimizing multiple textures into a single image. It use a custom format `.fe.atlas`. See [here for more detail](./ressourceType/atlas.md)
...
- `Shader`: Represents shader programs used for rendering effects. It can be in two formats : GLSL and HLSL.
...
- `Localization`: Represents localization files used for supporting multiple languages. It can be in various formats : JSON, XML, YAML, CSV. See [more informations here](./ressourceType/localization.md)
...
- `Mesh (future proof)`: Represents 3D mesh resources used for rendering 3D models. It can be in various formats : OBJ, FBX, GLTF.
    > [!WARNING]
    > The `Mesh` resource type is planned for future implementation and is not currently supported.
...
## Ideas
### `Ressource<Aseprite>`
In a future proof for my own game, we can generate a new `RessourceType` specialized into parsing [Aseprite]() format.